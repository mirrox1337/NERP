...
AddEventHandler('esx_loffe_fangelse:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
    ...
    CurrentActionMsg  = ('Tryck på ~INPUT_CONTEXT~ för att packa en låda')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Tryck på ~INPUT_CONTEXT~ för att lämna lådan.')
	CurrentActionData = {}
  elseif zone == 'TeleportIn' then
    CurrentAction	  = 'teleport_in'
	CurrentActionMsg  = ('Tryck på ~INPUT_CONTEXT~ för att gå in i lagerhuset.')
	CurrentActionData = {}
  elseif zone == 'TeleportUt' then
    CurrentAction	  = 'teleport_ut'
	CurrentActionMsg  = ('Tryck på ~INPUT_CONTEXT~ för att gå in i fängelset.')
	CurrentActionData = {}
  end
end)
...
AddEventHandler('esx_loffe_fangelse:startJob', function()
	...
	local SpawnObject = CreateObject(HashKeyOpenBox, 1688.65, 2551.49, 45.70)
	SetEntityCoords(GetPlayerPed(-1), 1690.16, 2551.56, 44.60)
	SetEntityHeading(GetPlayerPed(-1), 92.05)
    TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_BUM_BIN", 0, true)
	CanLeaveBox = true
	TimeLeft = Config.TimeToPackBox
	repeat
	TriggerEvent("mt:missiontext", '~w~Packar lådan. Tid kvar: ~p~' .. TimeLeft .. ' ~w~sekunder', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
