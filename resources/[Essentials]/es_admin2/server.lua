...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		if isBanned(v) then
			set(GetConvar("es_admin_banreason", "Du har blivit bannnad!"))
			CancelEvent()
			break
		end
	end
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "SYSTEM", {255, 0, 0}, "Du har inte behöveriheterna")
			end
		end)
	end)
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("es_admin_banreason", "Du har blivit bannad!"))
						end
					else
						if not available then
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "Du har inte rättigheterna.")
						else
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "Du har inte rättigheterna.")
						end
					end
...
end, {help = "Anmäl en spelare eller ett problem.", params = {{name = "report", help = "Vad vill du anmäla?"}}})
	...
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				table.remove(reason, 1)
				if(#reason == 0)then
					reason = "Kickad: Du blev kickad"
				else
					reason = "Kickad: " .. table.concat(reason, " ")
				end

				TriggerClientEvent('chatMessage', -1, "SYSTEM", {255, 0, 0}, "^2" .. GetPlayerName(player) .. "^0 blev kickad(^2" .. reason .. "^0)")
