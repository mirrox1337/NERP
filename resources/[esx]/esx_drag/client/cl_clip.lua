...
local isGettingDragged                 = false
local sourcePed                    = 0

RegisterNetEvent('esx_drag:drag')
AddEventHandler('esx_drag:drag', function(sri)
  --TriggerServerEvent('esx:clientLog', 'starting dragging')
  isGettingDragged = not isGettingDragged
  sourcePed = tonumber(sri)
end)

Citizen.CreateThread(function()
  while true do
    Wait(0)
	if isGettingDragged then
	  local ped = GetPlayerPed(GetPlayerFromServerId(sourcePed))
