...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = _U('have_withdrawn', ESX.Math.GroupDigits(amount)) })
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = _U('invalid_amount') })
		end
	end)
end)
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = _U('have_deposited', ESX.Math.GroupDigits(amount)) })
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = _U('invalid_amount') })
	end
end)
...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if amount and amount > 0 and account.money >= amount then
		...
		MySQL.Async.execute('INSERT INTO society_moneywash (identifier, society, amount) VALUES (@identifier, @society, @amount)', {
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = _U('you_have', ESX.Math.GroupDigits(amount)) })
		end)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = _U('invalid_amount') })
	end
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'success', text = _U('you_have_been_hired', job) })
			elseif type == 'promote' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'success', text = _U('you_have_been_promoted') })
			elseif type == 'fire' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'error', text = _U('you_have_been_fired', xTarget.getJob().label) })
			end
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'success', text = _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)) })
			end
