...
local robbableItems = {
 [1] = {x = 1.90339700, y = -3.80026800, z = 1.29917900, name = "Kylskåp", isSearched = false},
 [2] = {x = -3.50363200, y = -6.55289400, z = 1.30625800, name = "Nattduksbord", isSearched = false},
 [3] = {x = -3.50712600, y = -4.13621600, z = 1.29625800, name = "Bord", isSearched = false},
 [4] = {x = 8.47819500, y = -2.50979300, z = 1.19712300, name = "Byrå", isSearched = false},
 [5] = {x = 9.75982700, y = -1.35874100, z = 1.29625800, name = "Byrå", isSearched = false},
 [6] = {x = 8.46626300, y = 4.53223600, z = 1.19425800, name = "Garderob", isSearched = false},
 [7] = {x = 5.84416200, y = 2.57377400, z = 1.22089100, name = "Bord", isSearched = false},
 [8] = {x = 3.04164100, y = 0.31671810, z = 3.58363900, name = "Bokhylla", isSearched = false},
 [9] = {x = 6.86376900, y = 1.20651200, z = 1.36589100, name = "Under Sängen", isSearched = false},
 [10] = {x = 2.03442400, y = -5.61585100, z = 3.30395600, name = "Skänk", isSearched = false},
 [11] = {x = -5.53120400, y = 0.76299670, z = 1.77236000, name = "Skåp", isSearched = false},
 [12] = {x = -1.24716200, y = 1.07820500, z = 1.69089300, name = "TV-bord", isSearched = false},
 ...
 if lockpicks > 0 and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    exports["t0sic_loadingbar"]:StartDelayedFunction("Dyrkar upp lås", 12000, function()
     isLockpicking = false
     pedSpawned = false
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   showMessage('Tryck ~g~E~w~ för att gå ut')
...
function isEnoughCops()
  ESX.TriggerServerCallback('houseRobberies:anycops', function(anycops)
    if anycops >= 1 then
      return true
    else
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Tryck ~g~H~s~ för att leta igenom ' .. myRobbableItems[i]["name"])
     ...
     if IsControlJustReleased(1, 74) then
      ...
      exports["t0sic_loadingbar"]:StartDelayedFunction("Letar i "..myRobbableItems[i]['name'], 20000, function()
