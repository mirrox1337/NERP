      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You already have a same item in your shop, but for ' .. data[1].price .. '. you put the price ' .. Price, style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
                Wait(250)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'Remove the item and put a new price or put the same price', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'inform', text = 'You don\'t have enough money.', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
            elseif ItemCount <= 0 then
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'inform', text = 'invalid quantity.', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You bought ' .. ItemCount .. 'x ' .. Item .. ' for $' .. ItemCount * result[1].price, style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'You ordered' .. count .. ' pieces ' .. label .. ' for $' .. price * count, style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'You don\'t have enough money in your shop.', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'You bought a shop for $' ..  price, style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
        else    
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'You can\'t afford this shop', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You put in $' .. amount .. ' in your shop', style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
        
        else
        TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You can\'t put in more than you own', style = { ['background-color'] = '#b00000', ['color'] = '#fff' } })
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You took out $' .. amount .. ' from your shop', style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
            xPlayer.addMoney(amount)
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'You can\'t put in more than you own', style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
        end
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You sold your shop', style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
            else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You can\'t sell your shop with items or money inside of it', style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
            end
        end)
    end)
end)
    ...
    for i=1, #ESX.GetPlayers(), 1 do
        local identifier = ESX.GetPlayerFromId(ESX.GetPlayers()[i])
            ...
            if result[1].identifier == identifier.identifier then
                TriggerClientEvent('mythic_notify:client:SendAlert', identifier.source, { type = 'inform', text = msg, style = { ['background-color'] = '#009c10', ['color'] = '#fff' } })
            end
