...
ESX.RegisterServerCallback('esx_mecanojob:takelockpick', function(source, cb)
  local amount = 0
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mecano', function(account)
    if account.money >= amount then
      ...
      cb(false)
      local xPlayer = ESX.GetPlayerFromId(source)
      TriggerClientEvent('esx:showNotification', xPlayer.source, 'Du tog ut ett dyrkset' )
    end
  end)
end)

ESX.RegisterServerCallback('esx_mecanojob:takenyckel', function(source, cb)
  local amount = 0
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mecano', function(account)
    if account.money >= amount then
      account.removeMoney(amount)
      local xPlayer = ESX.GetPlayerFromId(source)
      xPlayer.addInventoryItem('mecanonyckel', 1)
      cb(true)
    else
      cb(false)
      local xPlayer = ESX.GetPlayerFromId(source)
      TriggerClientEvent('esx:showNotification', xPlayer.source, 'Du tog ut en nyckel' )
    end
  end)
end)
