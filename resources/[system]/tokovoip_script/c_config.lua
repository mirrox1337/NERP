TokoVoipConfig = {
	...
	keyProximity = Keys["INSERT"], -- Keybind used to switch the proximity mode
	...
	plugin_data = {
		...
		TSServer = "92.35.201.189", -- TeamSpeak server address to be displayed on blocking screen
		...
		localName = "[NERP] PLAYER", -- If set, this name will be used as the user's teamspeak display name
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = escape((PlayerId())); -- Set the local name
		end);
