...
function openMenu()
	...
	table.insert(elements, {label = 'Köp en laseroperation 1000 SEK', value = 'laser'})
	...
	function(data, menu)
		...
		if data.current.value == 'laser' then
			TriggerServerEvent("esx_tattooshop:laser", 1000)
		else
			elements = {}

			table.insert(elements, {label = _U('go_back_to_menu'), value = nil})
			for i,k in pairs(tattoosList[data.current.value]) do
				table.insert(elements, {label= _U('tattoo') .. " nummer "..i.." - " .. _U('money_amount', k.price), value = i, price = k.price})
			end
