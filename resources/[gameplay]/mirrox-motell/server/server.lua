...
AddEventHandler('lsrp-motels:cancelRental', function(room)
    ...
    MySQL.Sync.execute("DELETE FROM motell WHERE ident=@ident AND motel_id=@roomno", {['@ident'] = playerIdent, ['@roomno'] = room})
...
AddEventHandler('lsrp-motels:rentRoom', function(room)
    ...
    MySQL.Sync.execute("INSERT INTO motell (ident, motel_id) VALUES (@ident, @roomno)", {['@ident'] = playerIdent, ['@roomno'] = room})
    

end)

--Kollar om någon redan hyr rummet
ESX.RegisterServerCallback('lsrp-motels:isMotelTaken', function(source, cb, room)
	local src = source
	MySQL.Async.fetchAll("SELECT COUNT(*) as count FROM motell WHERE motel_id = @room", {['@room'] = tostring(room)}, 
	function(status)
		cb(status[1].count)
	end)
end)
...
ESX.RegisterServerCallback('lsrp-motels:getMotelRoomID', function(source, cb, room)
    ...
    MySQL.Async.fetchScalar("SELECT id FROM motell WHERE ident=@ident AND motel_id = @room", {['@ident'] = playerIdent, ['@room'] = room}, function(rentalID)
...
ESX.RegisterServerCallback('lsrp-motels:checkOwnership', function(source, cb)
    ...
    if pid ~= nil then
        ...
        MySQL.Async.fetchScalar("SELECT motel_id FROM motell WHERE ident = @ident", {['@ident'] = playerIdent}, function(motelRoom)
...
ESX.RegisterServerCallback('lsrp-motels:checkIsOwner', function(source, cb, room, owner)
    ...
    MySQL.Async.fetchScalar("SELECT motel_id FROM motell WHERE motel_id = @room AND ident = @id", {
        ['@room'] = room,
...
--[[-- Testar med att ta bort hyran då den verkar crasha servern vid kl 22.
function PayRent(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM motell', {}, function (result)
		for i=1, #result, 1 do
