...
local radar =
{
	info = "~y~Laddar Radarn...~w~321...~y~Laddad! ",
}

Citizen.CreateThread( function()
	while true do
		Citizen.Wait(1) 
			if IsControlJustPressed(1, cfg.menuopen) then --311 = K
     			if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(cfg.radargun) then
					if shown == true then 
						shown = false 
						radar.info = string.format("~y~Ställer in Radarn...~w~321...~y~Laddad! ")
					else 
						shown = true 	
     				end
   				end
		...
		if shown then
				...
				if IsEntityAVehicle(e) then
					if IsPlayerFreeAiming(playerId) then
						...
						if cfg.metric == true then
							...
							radar.info = string.format("~y~Plåt: ~w~%s  ~y~Modell: ~w~%s  ~y~Hastighet: ~w~%s km/h", fplate, fmodel, math.ceil(fvspeed))
						else 
							local fvspeed = GetEntitySpeed(e)*3.6 -- m/s to mph
							local fplate = GetVehicleNumberPlateText(e)
							radar.info = string.format("~y~Plåt: ~w~%s  ~y~Modell: ~w~%s  ~y~Hastighet: ~w~%s km/h", fplate, fmodel, math.ceil(fvspeed))
						end
					end
			...
			DrawAdvancedText(0.600, 0.903, 0.005, 0.0028, 0.4, "Laserpistol", 0, 191, 255, 255, 6, 0)
