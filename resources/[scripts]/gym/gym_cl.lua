        ...
        local coords = GetEntityCoords(PlayerPedId())
            for i, v in pairs(Config.Locations) do
                ...
                if dist <= 1.5 and not exercising then
                    sleep = 5
                    DrawText3D(pos["x"], pos["y"], pos["z"] + 0.98, "[E] ~p~" .. pos["exercise"])
                    ...
                    elseif membership == false then
                        ...
                        sleep = 8
                        DrawText3D(pos["x"], pos["y"], pos["z"] + 0.98, "~p~" .. pos["exercise"])
                    end
                end
            end
        Citizen.Wait(sleep)
...
function startExercise(animInfo, pos)
    ...
    Citizen.CreateThread(function()
        while exercising do
            ...
            --DrawText2D(0.505, 0.925, 1.0,1.0,0.33, "~p~Procent: " .. color..procent .. "%", 255, 255, 255, 255)
            DrawText3D(pos["x"], pos["y"], pos["z"] + 0.98, "~p~Procent: " .. color..procent .. "%")
            DrawText2D(0.505, 0.95, 1.0,1.0,0.33, "Tryck ~g~[SPACE]~w~ för att träna", 255, 255, 255, 255)
            DrawText2D(0.505, 0.975, 1.0,1.0,0.33, "Tryck ~r~[DELETE]~w~ för att avsluta övningen", 255, 255, 255, 255)
        end
    end)
    ...
    Citizen.CreateThread(function()
        while canExercise do
            ...
            if procent <= 99 then
                ...
                ExitTraining(animInfo["exitDict"], animInfo["exitAnim"], animInfo["exitTime"])
                exports["gamz-skillsystem"]:UpdateSkill("Styrka", 0.5)
            end
        end
    end)
end
...
RegisterCommand("motion", function()
    ...
    Citizen.CreateThread(function()
        while doingMotion do
            ...
            DrawText2D(0.505, 0.95, 1.0,1.0,0.4, "~p~Konditionträning:~w~ " .. tonumber(string.format("%.1f", motionProcent/1000)) .. "%", 255, 255, 255, 255)
