	...
	if dna.p == nil then
		...
		if IsPedFatallyInjured(ped) then
			...
			if weapon ~= nil then
				...
				sendNotification('Du tog ett DNA test från den döda kroppen', 'error', 2500)
			else
				sendNotification('Du kunde inte hitta något DNA', 'error', 2500)
			end
		else
			dna = {k = nil, d = nil, w = nil, p = GetPlayerServerId(player)}
			sendNotification('Du tog DNA Från personen', 'error', 2500)
		end
	else
		sendNotification('Du har redan ett DNA Prov på dig ladda upp det i Databasen', 'error', 5000)
	end
end)
