...
--     Citizen.Wait(2000)
--     TriggerServerEvent('esx_addons_chrono:startCall', 'police', 'lol', {
--       x = 13,
--       y = 254,
--       z = 0
--     })
...
RegisterNetEvent('esx_addons_chrono:removeAlert')
AddEventHandler('esx_addons_chrono:removeAlert', function(alertid)
  removeAlert(alertid)
end)


RegisterNetEvent('esx_addons_chrono:notifyAlert')
AddEventHandler('esx_addons_chrono:notifyAlert', function(alert)
  local find = false
...
RegisterNetEvent('esx_addons_chrono:setPhoneNumberSource')
AddEventHandler('esx_addons_chrono:setPhoneNumberSource', function(phoneNumber, source)
  if source == -1 then
...
RegisterNUICallback('send', function(data)
  ...
  TriggerServerEvent('esx_addons_chrono:send', phoneNumber, data.message, data.anonyme, {
    x = coords.x,
    y = coords.y,
    z = coords.z
  })
...
RegisterNetEvent('esx_addons_chrono:showMessage')
AddEventHandler('esx_addons_chrono:showMessage', function (message)
  --ESX.ShowNotification(message)
  exports['mythic_notify']:DoCustomHudText('success', message, 5000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
end)
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if #alerts ~= 0 then
      ...
      SetTextColour(255, 255, 255, 255)
      SetTextEntry("STRING")
      AddTextComponentString(alert.isAccept .. ' enheter')
      DrawText(0.195, 0.02)
      ...
      if alert.message ~= nil then
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString("~o~Inget meddelande")
        DrawText(0.12, 0.08)
      end
      ...
      if alert.coords ~= nil then
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString('~o~Position okänt')
        DrawText(0.12, 0.14)
      end
      ...
      if alert.numero ~= nil then
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString('~o~Nummer okänt')
        DrawText(0.17, 0.14)
      end
      ...
      if GetLastInputMethod(2) then 
        ...
        elseif IsControlJustPressed(1, Keys['PAGEUP']) then
          TriggerServerEvent('esx_addons_chrono:acceptAlert', alert.type, alert.id)
          if alert.coords ~= nil then
        ...
        elseif IsControlJustPressed(1, Keys['PAGEDOWN']) then
          print('Press Y')
          TriggerServerEvent('esx_addons_chrono:refuseAlert', alert.type, alert.id)
...
RegisterNetEvent('esx_addons_chrono:call')
AddEventHandler('esx_addons_chrono:call', function(data)
  local playerPed   = GetPlayerPed(-1)
  local coords      = GetEntityCoords(playerPed)
  ...
  if message ~= nil and message ~= "" then
    TriggerServerEvent('chrono:sendMessage', number, message)
    Wait(20)
