...
AddEventHandler('qalle:job', function(number, message, position, caller, dispatchRequestId)
  ...
  if number == tonumber(number) then
    exports['mythic_notify']:DoCustomHudText('success', 'Du fick ett meddelande!', 4500 , { ['background-color'] = '#007ecc', ['color'] = '#fff' })
    PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", 0, 0, 1)
    Citizen.Wait(250)
    job = number
  elseif number ~= tonumber(number) then

    exports['mythic_notify']:DoCustomHudText('success', "[Till: " .. numbertext .. " Från: " .. caller .. "] " .. message, 10000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
  ...
  if (dispatchRequestId) then
    ...
    CurrentActionMsg         = exports['mythic_notify']:DoCustomHudText('success', numbertext .. ' - Tryck [Y] för svara på samtalet', 10000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
...
AddEventHandler("chrono:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    exports['mythic_notify']:DoCustomHudText('success', text, 5000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
...
AddEventHandler('chrono:stopDispatch', function(dispatchRequestId, playerName, policeDispatch)
  if CurrentDispatchRequestId == dispatchRequestId then
    ...
    exports['mythic_notify']:DoCustomHudText('success', playerName.firstname .. " tar det samtalet. ", 3500, { ['background-color'] = '#119100', ['color'] = '#fff' })
  end
end)
...
AddEventHandler('chrono:stopDispatch2', function(dispatchRequestId, playerName, policeDispatch)
  if CurrentDispatchRequestId == dispatchRequestId then
    ...
    exports['mythic_notify']:DoCustomHudText('success', playerName.firstname .. " åker på det larmet. ", 3500, { ['background-color'] = '#119100', ['color'] = '#fff' })
  end
end)
