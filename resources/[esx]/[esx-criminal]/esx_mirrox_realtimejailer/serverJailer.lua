		...
		if result[1] ~= nil then
			if result[1].timeleft > 0 then
				if result[1].cellule ~= nil then
					TriggerClientEvent("esx_mirrox_jailer:JailInStation", source,result[1].cellule,result[1].timeleft)
				end
...
RegisterServerEvent("esx_mirrox_jailer:JailUpdate")
AddEventHandler("esx_mirrox_jailer:JailUpdate", function(newTime)
	...
	if newTime == 0 then
		...
		TriggerClientEvent("esx_mirrox_jailer:UnJail", playerid)
	end
end)
...
RegisterServerEvent("esx_mirrox_jailer:PutInJail")
AddEventHandler("esx_mirrox_jailer:PutInJail", function(playerid, jail, jailtime)
	--calculate jailtime
	...
	if jail ~= nil then
			print("[esx_mirrox_jailer] Sätt i fängelse i ".. jail.." :".. GetPlayerName(playerid).. " i ".. jailtime .." sekunder - av ".. GetPlayerName(source))
			TriggerEvent('esx:importantlogs',"[esx_mirrox_jailer] Sätt i fängelse i ".. jail.." :".. GetPlayerName(playerid).. " i ".. jailtime .." sekunder - av ".. GetPlayerName(source))
			...
			TriggerClientEvent("esx_mirrox_jailer:JailInStation", playerid,jail,jailtime)
	end
end)

RegisterServerEvent("esx_mirrox_jailer:UnJailplayer")
AddEventHandler("esx_mirrox_jailer:UnJailplayer", function(playerid)
	local identifier = GetPlayerIdentifiers(playerid)[1]
...
RegisterServerEvent("esx_mirrox_jailer:UnJailplayer2")
AddEventHandler("esx_mirrox_jailer:UnJailplayer2", function()
	local identifier = GetPlayerIdentifiers(source)[1]
	if GetPlayerName(source) ~= nil then
		TriggerClientEvent("esx_mirrox_jailer:UnJail", source)
		-- MySQL.Async.execute("INSERT INTO jail (identifier,J_Time, isjailed) VALUES (@Identifier,@J_Time,@isjailed) ON DUPLICATE KEY UPDATE Identifier=identifier,isjailed=@isjailed,J_Time=@J_Time", {['@identifier'] = identifier, ['@J_Time'] = os.date('%Y-%m-%d %H:%M:%S'),['@isjailed'] = false})
		-- MySQL.Async.execute("UPDATE jail SET `isjailed` = false where identifier = @identifier", {['@identifier'] = identifier)
		MySQL.Async.execute("DELETE FROM jail WHERE identifier=identifier", {['@identifier'] = identifier})
	end
