...
RegisterCommand("tow", function(source, args)
    ...
    if towing then
        ...
        elseif not towSetupMode then
            ShowNotification("Gå till din ~r~Bärgningsbil ~s~och när du ser den", "~y~gula ~s~markern, tryck ~r~~h~E~h~", "~s~för att välja fordonet.")
            towSetupMode = true
            towtruck = nil
            target = nil
            helpstate = 1
...
Citizen.CreateThread(function()
    while true do
        if towSetupMode then
            ...
            if helpstate ~= 0 then
                ...
                if hit and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if (IsControlJustPressed(0, 206)) then
                        if helpstate == 1 then
                            ShowNotification("Gå till den bil som ska ~o~bogseras ~s~och när du ser den", "~y~gula ~s~markern, tryck ~r~~h~E~h~", "~s~för att bekräfta att fordonet ska bogseras.")
                            towtruck = veh
                            helpstate = 2
                        elseif helpstate == 2 and veh ~= towtruck then
                            ShowNotification("Fordon har bekräftats.", "~r~/tow ~s~att starta bogsering, eller tryck", "~r~~h~E~h~~s~ för att avbryta.")
                            target = veh
                            helpstate = 3
                        end
                    end
            ...
            elseif helpstate == 0 and IsControlJustPressed(0, 206) and towtruck ~= nil and target ~= nil then
                towtruck = nil
                target = nil
                helpstate = 1
            end
