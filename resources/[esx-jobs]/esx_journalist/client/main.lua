  ...
  Citizen.CreateThread(function ()
	while true do
	  ...
	  if CurrentAction ~= nil then
		...
		AddTextComponentString(CurrentActionMsg)
		DisplayHelpTextFromStringLabel(0, 0, 1, -1)
		Draw3DText(-1048.4, -229.44, 40.01, '[~g~E~w~] för att hämta/lämna ut/in nyckel')
		if IsControlJustReleased(0, Keys['E']) then
			 if CurrentAction == 'police_card' then
			  if PlayerData.job.name == 'journaliste' then
			  TriggerServerEvent('card:giveItem')
			  else 
				sendNotification('Du är inte anställd inom NE Nyheter.', 'error', 2300)
			  end
		   end
	   end
	  end
