...
local blips = {
  {title="NERPOnline Pizzaleverans", colour=51, id=103, x = -1285.73, y = -1387.15, z = 3.44}, --Configuration du point sur la carte
}
  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Tryck på ~INPUT_CONTEXT~ för att börja leverera ~r~pizza",0,1,0.5,0.8,0.6,255,255,255,255)
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          text = "Destination : " ..destinol.. ", Leverera dit!",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "bottomRight"
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        HelpText("Tryck på ~INPUT_CONTEXT~ för att leverera pizzor!",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if (posibilidad > 1) and (posibilidad < 40) then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Här du får en bonus för snabb leverans! : " .. pourboire .. "SEK",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
    ...
    if isToPizzaria == true then
      ...
      if GetDistanceBetweenCoords(pizzeria.x,pizzeria.y,pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaitunepizzamin == true then
        HelpText("Tryck på ~INPUT_CONTEXT~ för att hämta ut pizzorna!",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerEvent("pNotify:SendNotification", {
                text = "Bra jobbat! Vi har några leveranser kvar!",
                type = "success",
                queue = "global",
                timeout = 4000,
                layout = "bottomRight"
              ...
              while notifmoto1 == true do
                ...
                TriggerEvent("pNotify:SendNotification", {
                  text = "Har du glömt bort mopeden?",
                  type = "error",
                  queue = "global",
                  timeout = 4000,
                  layout = "bottomRight"
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      if GetDistanceBetweenCoords(pizzeriafin.x, pizzeriafin.y, pizzeriafin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Tryck på ~INPUT_CONTEXT~ för att stoppa leveransen av ~r~pizzan",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if afaitunepizzamin == true then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Tack för att du jobbat idag! Här får du din lön: " .. leverans .. "SEK",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Tack ändå, ha en trevlig dag",
              type = "error",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
