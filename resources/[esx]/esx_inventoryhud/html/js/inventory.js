...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
		...
		$("#inventory").html(data.inventory + "/10kg");
    });
});
...
$(document).ready(function () {
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "housing" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://esx_inventoryhud/TakeFromHousing", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "housingbed" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://esx_inventoryhud/TakeFromHousingBed", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "housing" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://esx_inventoryhud/PutIntoHousing", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "housingbed" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://esx_inventoryhud/PutIntoHousingBed", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
