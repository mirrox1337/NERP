...
function YellowPostPages (phone_number, firstname, lastname, message, sourcePlayer, cb)
    ...
    MySQL.Async.insert("INSERT INTO yellow_tweets (`phone_number`, `firstname`, `lastname`, `message`) VALUES(@phone_number, @firstname, @lastname, @message);", {
      ...
      MySQL.Async.fetchAll('SELECT * from yellow_tweets WHERE id = @id', {
        ...
        TriggerClientEvent('chrono:yellow_newPagess', -1, pages)
        TriggerEvent('chrono:yellow_newPagess', pages)
      end)
    end)
  end)
end
...
function YellowShowError (sourcePlayer, title, message)
  TriggerClientEvent('chrono:yellow_showError', sourcePlayer, message)
end
function YellowShowSuccess (sourcePlayer, title, message)
  TriggerClientEvent('chrono:yellow_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('chrono:yellow_getPagess')
AddEventHandler('chrono:yellow_getPagess', function(phone_number, firstname)
  ...
  if phone_number ~= nil and phone_number ~= "" and firstname ~= nil and firstname ~= "" then
    getUserYellow(phone_number, firstname, function (user)
      ...
      YellowGetPagess(accountId, function (pagess)
        TriggerClientEvent('chrono:yellow_getPagess', sourcePlayer, pagess)
      end)
    end)
  else
...
RegisterServerEvent('chrono:yellow_postPagess')
AddEventHandler('chrono:yellow_postPagess', function(firstname, phone_number, lastname, message)
  local sourcePlayer = tonumber(source)
...
RegisterServerEvent('chrono:deleteYellow')
AddEventHandler('chrono:deleteYellow', function(id)
    deleteYellow(id)
end)
