...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Dölj alla butiker på kartan', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Visa alla butiker på kartan', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Du har: SEK' .. data[1].money .. ' i ditt företag',    value = ''})
		table.insert(elements, {label = 'Leveranser',    value = 'shipments'})
        table.insert(elements, {label = 'Lägg in en vara till försäljning', value = 'putitem'})
        table.insert(elements, {label = 'Ta ut en vara till försäljning',    value = 'takeitem'})
        table.insert(elements, {label = 'Lägg in pengar i din företag',    value = 'putmoney'})
        table.insert(elements, {label = 'Ta ut pengar från ditt företag',    value = 'takemoney'})
        table.insert(elements, {label = 'Byt namn på ditt företag: SEK' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = 'Sälj ditt företag för SEK' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
		...
		menu2.close()
	 	...
	 	elseif data.current.value == 'putmoney' then
			...
			title = 'Hur mycket vill du lägga in?'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Beställ produkt', value = 'buy'})
	table.insert(elements, {label = 'Transport', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--Klar för transport--'})
			table.insert(elements, {label = 'Få alla dina leveranser', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--Sändningar--'})
				once = false
			end

			if time - items[i].time <= Config.DeliveryTime then
				times = time - items[i].time
				table.insert(elements, {label = items[i].label .. ' Tid kvar: ' .. math.floor((Config.DeliveryTime - times) / 60) .. ' Minuter' })
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' för ' .. Config.Items[i].price .. ' SEK  ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
    ...
    for i=1, #result.items, 1 do
      ...
      local invitem = result.items[i]
	    ...
	    if invitem.count > 0 then
			table.insert(elements, { label = invitem.label .. ' | ' .. invitem.count .. ' I din väska', count = invitem.count, name = invitem.name})
	    end
	end
...
Citizen.CreateThread(function()
	Citizen.Wait(500)
		...
		AddTextComponentString('Affärsfastigheter')
...
function Robbery(id)
	...
	local coords1 = {
		...
		[4] = {x = -1478.97, y = -374.36, z = 39.16, heading = 228.54},
		[5] = {x = 1395.49, y = 3612.94, z = 34.85, heading = 22.18},
		[6] = {x = -2959.5, y = 387.78, z =13.85 , heading = 171.58},
		[7] = {x = 2674.36, y = 3288.0, z = 54.24, heading = 339.44},
		[8] = {x = -42.76, y = -1749.36, z = 29.32, heading = 320.15},
		[9] = {x = 1160.77, y = -314.03, z = 68.91, heading = 10.81},
		[10] = {x = -708.14, y = -904.05, z =19.12 , heading = 5.29},
		[11] = {x = -1828.23, y = 799.83, z = 138.06, heading = 53.63},
		[12] = {x = 1706.87, y = 4919.76, z = 41.96, heading = 237.41},
		[13] = {x = 1960.75, y = 3748.67, z = 32.24, heading = 304.34},
		[14] = {x = 1126.26, y = -980.84, z = 45.12, heading = 9.13},
		[15] = {x = 30.45, y = -1339.88, z = 29.05 , heading = 269.79},
		[16] = {x = -1383.59, y = -630.25, z = 29.82, heading = 213.19},
		[17] = {x = 545.07, y = 2663.47, z = 42.06, heading = 96.45},
		[18] = {x = -3249.02, y = 1006.04, z = 12.83, heading = 0.76},
		[19] = {x = 1168.51, y = 2718.37, z = 36.96, heading = 271.45},
		[20] = {x = 1736.66, y = 6419.02, z = 34.94, heading = 247.6},
    }
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "Bryt upp kassaskåpet: " .. hp/50 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", 'Rånet på din butik	(' .. Name .. ') var tyvärr framgångsrikt!', Id)
					DeleteEntity(Var)
				end
