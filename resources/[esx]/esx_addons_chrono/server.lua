...
function notifyAlert(number, alert) 
  ...
  if PhoneNumbers[number] ~= nil then
    for k,v in pairs(PhoneNumbers[number].sources) do
      TriggerClientEvent('esx_addons_chrono:notifyAlert', tonumber(k), alert)
    end
  end
end
    ...
    for k,v in pairs(PhoneNumbers[number].sources) do
      getPhoneNumber(tonumber(k), function (n)
        ...
        if alert.coords ~= nil then
          TriggerEvent('esx_addons_chrono:_internalAddMessage', number, n, 'GPS: ' .. alert.coords.x .. ', ' .. alert.coords.y, 0, function (smsMess)
            TriggerClientEvent("esx_addons_chrono:receiveMessage", tonumber(k), smsMess)
          end)
        end
      end)
...
AddEventHandler('esx_phone:registerNumber', function(number, type, sharePos, hasDispatch, hideNumber, hidePosIfAnon)
  print('==== Registrerade telefonnummer ' .. number .. ' => ' .. type)
...
AddEventHandler('esx:setJob', function(source, job, lastJob)
  if PhoneNumbers[lastJob.name] ~= nil then
    TriggerEvent('esx_addons_chrono:removeSource', lastJob.name, source)
  end

  if PhoneNumbers[job.name] ~= nil then
    TriggerEvent('esx_addons_chrono:addSource', job.name, source)
  end
end)

AddEventHandler('esx_addons_chrono:addSource', function(number, source)
  PhoneNumbers[number].sources[tostring(source)] = true
end)

AddEventHandler('esx_addons_chrono:removeSource', function(number, source)
  PhoneNumbers[number].sources[tostring(source)] = nil
end)


RegisterServerEvent('esx_addons_chrono:startCall')
AddEventHandler('esx_addons_chrono:startCall', function (number, message, coords)
  local source = source
  if PhoneNumbers[number] ~= nil then
...
RegisterServerEvent('esx_addons_chrono:acceptAlert')
AddEventHandler('esx_addons_chrono:acceptAlert', function (type, alertId)
  print('esx_addons_chrono:acceptAlert', type, alertId)
  local source = source
  if PhoneNumbers[type] ~= nil then
    print('esx_addons_chrono:acceptAlert P1')
    for key, alert in pairs(PhoneNumbers[type].alerts) do
      print('alert check', alert.id)
      if alert.id == alertId then
        print('esx_addons_chrono:acceptAlert P2')
        PhoneNumbers[type].alerts[key].isAccept = PhoneNumbers[type].alerts[key].isAccept + 1
        --TriggerClientEvent('esx_addons_chrono:showMessage', tonumber(key), PhoneNumbers[type].type .. "Vi har skickat enhet: " .. alert.firstN .. '-' .. alert.secondN .. '. Vänligen stanna kvar på platsen.')
...
RegisterServerEvent('esx_addons_chrono:refuseAlert')
AddEventHandler('esx_addons_chrono:refuseAlert', function (type, alertId)
  local source = source
  if PhoneNumbers[type] ~= nil then
    for key, alert in pairs(PhoneNumbers[type].alerts) do
      if alert.id == alertId then
        print('esx_addons_chrono:refuseAlert find')
        ...
        if allRefuse == true then
          if PhoneNumbers[type].alerts[key].isAccept == 0 then
              --TriggerClientEvent('esx_addons_chrono:showMessage', tonumber(key), 'Alla enheter ignorerar samtal')
            for k, src in pairs(PhoneNumbers[type].alerts[key].sources) do 
              --TriggerClientEvent('esx_addons_chrono:showMessage', tonumber(k), 'Alla enheter ignorerar samtal')
            end
          end
...
AddEventHandler('esx:playerLoaded', function(source)
  ...
  MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier',{
    ...
    if PhoneNumbers[xPlayer.job.name] ~= nil then
      TriggerEvent('esx_addons_chrono:addSource', xPlayer.job.name, source)
    end
  end)
...
AddEventHandler('esx:playerDropped', function(source)
  ...
  local xPlayer = ESX.GetPlayerFromId(source)
  if PhoneNumbers[xPlayer.job.name] ~= nil then
    TriggerEvent('esx_addons_chrono:removeSource', xPlayer.job.name, source)
  end
  ...
  for number, infoPhone in pairs(PhoneNumbers) do 
    ...
    for ka, alerts in pairs(PhoneNumbers[number].alerts) do 
      if tonumber(ka) == source then
        ...
        TriggerClientEvent('esx_addons_chrono:removeAlert', -1, PhoneNumbers[number].alerts[ka].id)
